...
namespace NetDetector
{
    class Program
    {
        static int Main(string[] args)
        {
            try
            {
                var parser = new SimpleCli.Parser(args, version: "0.0.4", maxWidth: 65);
                ...
                var connect = parser["connect"].GetStringArray();
                var disconnect = parser["disconnect"].GetStringArray();
